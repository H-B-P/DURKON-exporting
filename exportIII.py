...
def model_to_lines(model, detail=1, filename="op.csv", specifiedConts = {}):
 ...
 for col in model['conts']:
  ...
  for i in range(len(contPuts)):
    lines[i+2] = lines[i+2] + ',,' + str(contPuts[i]) + ',' + str(apply_model.get_effect_of_this_cont_col_on_single_input(contPuts[i], model, col))
 ...
 for col in model['cats']:
  lines[0] = lines[0]+',,,'
  lines[1] = lines[1]+',,'+col+','
  keys = apply_model.get_sorted_keys(model, col)
